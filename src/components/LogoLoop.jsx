...
const useAnimationLoop = (trackRef, targetVelocity, seqWidth, seqHeight, isHovered, hoverSpeed, isVertical, offsetRef, isDraggingRef) => {
    ...
    useEffect(() => {
        ...
        const animate = timestamp => {
            ...
            const target = isDraggingRef.current ? 0 : (isHovered && hoverSpeed !== undefined ? hoverSpeed : targetVelocity);
...
export const LogoLoop = memo(({
    ...
    const handlePointerDown = useCallback((e) => {
        ...
        startPosRef.current = isVertical ? clientPos.clientY : clientPos.clientX;
    ...
    const handlePointerMove = useCallback((e) => {
        ...
        const clientPos = e.touches ? e.touches[0] : e;
        const currentPos = isVertical ? clientPos.clientY : clientPos.clientX;
        const diff = startPosRef.current - currentPos;
        const seqSize = isVertical ? seqHeight : seqWidth;
        if (seqSize > 0) {
            let nextOffset = lastOffsetRef.current + diff;
            nextOffset = ((nextOffset % seqSize) + seqSize) % seqSize;
            offsetRef.current = nextOffset;
        }
    ...
    const renderLogoItem = useCallback((item, key) => {
        ...
        const content = isNodeItem ? (
            <span className="logoloop__node" aria-hidden={!!item.href && !item.ariaLabel} style={{ pointerEvents: 'none' }}>{item.node}</span>
        ) : (
            <img src={item.src} srcSet={item.srcSet} sizes={item.sizes} width={item.width} height={item.height}
                alt={item.alt ?? ''} title={item.title} loading="lazy" decoding="async" draggable={false} style={{ pointerEvents: 'none' }} />
        );
        const itemAriaLabel = isNodeItem ? (item.ariaLabel ?? item.title) : (item.alt ?? item.title);
        const itemContent = item.href ? (
            <a className="logoloop__link" href={item.href} aria-label={itemAriaLabel || 'logo link'} target="_blank" rel="noreferrer noopener" style={{ pointerEvents: 'none' }}>{content}</a>
        ) : content;
        return <li className="logoloop__item" key={key} role="listitem" onDragStart={(e) => e.preventDefault()}>{itemContent}</li>;
    }, [renderItem]);

    const logoLists = useMemo(() =>
        Array.from({ length: copyCount }, (_, copyIndex) => (
            <ul className="logoloop__list" key={`copy-${copyIndex}`} role="list"
                aria-hidden={copyIndex > 0} ref={copyIndex === 0 ? seqRef : undefined}
    ...
    const containerStyle = useMemo(() => ({
        ...
        ...cssVariables, ...style,
        touchAction: 'none' // Prevent native scroll while touching
